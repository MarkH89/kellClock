...
function kellClock(selector, dateEndString) {
  var daysAsHours = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  var textContents = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {
    d: 'd',
    h: 'h',
    m: 'm',
    s: 's'
  };
  var minPadding = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 2;
  var debugMode = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;

  // Check required values have been passed
  if (!selector || !dateEndString) {
    return null;
  }
  ...
  _d._minPadding = minPadding; // Preflight setup
...
kellClock.prototype._preflight = function () {
  ...
  _d._container.dataset.clockStatus = "init"; // Create time fracts

  var _arr = Object.keys(_d._fracts);

  for (var _i = 0; _i < _arr.length; _i++) {
    var k = _arr[_i];
    _d._fracts[k] = document.createElement('span');
    _d._fracts[k].className = "".concat(_d._selector, "__").concat(k, " kellClock-fraction");
...
kellClock.prototype.run = function () {
  var _d = this._data; // Check clock is ready to go
  ...
  for (var k in _d._fracts) {
    if (k == 'd' && _d._daysAsHours) {
      // If Days is to be included as hours, then ignore the D element
      continue;
    }

    _d._container.appendChild(_d._fracts[k]); // Check text contents exists and insert if required
  ...
  _d._timeLeft = _d._dateEnd - _d._timeNow; // Check there's time left to go, otherwise, don't run the update!
...
kellClock.prototype._update = function () {
  ...
  var _d = this._data; // Calculate time remaining

  _d._timeNow = new Date().getTime();
  _d.timeLeft = _d._dateEnd - _d._timeNow; // If timer has expired...
  ...
  if (_d.timeLeft < 0) {
    ...
    _d._container.dataset.clockStatus = "running"; // Only update Days if not daysAsHours
    ...
    _d._fracts.s.textContent = this._stringPad(Math.floor(_d.timeLeft % _d._m / 1000)); // Set timeout to run for the next second
